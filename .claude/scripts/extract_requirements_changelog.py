...
def is_requirements_commit(scope: str, body: str) -> bool:
    ...
    if (
        ...
        or normalized.startswith("req")
        or normalized.startswith("requirements")
    ):
        return True

    changelog_target = parse_footer(body, "Changelog").lower()
    if changelog_target == "requirements":
        return True
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--changelog-file",
        default="examples/saas-webapp/requirements/CHANGELOG.md",
        help="Target changelog file used with --apply",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Insert generated section under '## Unreleased' in changelog file",
    )
