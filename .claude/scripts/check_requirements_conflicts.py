...
RE_ENDPOINT_RAW = re.compile(r"(/(?:api|messaging|leads|calls|client-portal|supervisor|settings|work|inbox|mycase)[^\s,)]*)")
RE_TICKET = re.compile(r"\bTC-\d{3}\b")
...
def parse_story(path: Path, root: Path) -> Story | None:
    ...
    parts = rel.parts
    # stories are expected as <requirements-root>/<area>/<screen>/<story>.md
    if len(parts) < 3:
        return None
...
def main() -> int:
    ...
    parser.add_argument("--root", default="examples/saas-webapp/requirements", help="Requirements root directory")
    parser.add_argument(
        "--range",
        default="HEAD~1..HEAD",
        help="Git range for --changed-only mode (default: HEAD~1..HEAD)",
    )
    parser.add_argument(
        "--changed-only",
        action="store_true",
        help="Only enforce overlap checks for stories changed in git range",
    )
    parser.add_argument(
        "--strict-warnings",
        action="store_true",
    ...
    # Endpoint overlap / possible duplication and conflicts
        ...
        for a, b in itertools.combinations(group, 2):
            ...
            if sim >= 0.75:
                errors.append(
                    ...
                    f"{a.path} and {b.path} share endpoint {ep} with high acceptance similarity ({sim:.2f})"
                )

            has_idempotent = "idempotent" in acc_a.lower() or "idempotent" in acc_b.lower()
            has_non_idempotent = (
                "non-idempotent" in acc_a.lower()
                or "non-idempotent" in acc_b.lower()
                or "not idempotent" in acc_a.lower()
                or "not idempotent" in acc_b.lower()
            )
            if has_idempotent and has_non_idempotent:
                errors.append(
                    "CONFLICTING ACCEPTANCE: "
                    f"{a.path} vs {b.path} on endpoint {ep} (idempotent vs non-idempotent)"
                )
