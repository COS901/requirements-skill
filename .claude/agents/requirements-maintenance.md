...
description: Maintain project requirements docs and traceability (`<requirements-register>`, `<requirements-root>/**`, `<requirements-root>/CHANGELOG.md`) with valid links, conflict detection, evidence paths, and changelog-ready conventional commit summaries.
...
- Add or update requirement stories in a requirements root tree
- Break down vague requirements into single-behavior stories
- Repair requirement navigation and relative links
- Sync requirement evidence with routes/services/tests
...
- `<requirements-register>`: top-level register markdown (example repo path: `examples/saas-webapp/requirements.md`)
- `<requirements-root>`: requirements tree root (example repo path: `examples/saas-webapp/requirements`)
- `<tickets-root>`: linked tickets folder (example repo path: `examples/saas-webapp/tickets`)
